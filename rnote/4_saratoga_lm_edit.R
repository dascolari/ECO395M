...
lmcrush = lm(price ~ (. - pctCollege - sewer - newConstruction) + lotSize*bedrooms*bathrooms, data = saratoga_test)

coef(lm1) %>% round(0)
coef(lm2) %>% round(0)
coef(lm3) %>% round(0)
coef(lmcrush) %>% round(0)

# Predictions out of sample
# Root mean squared error
# the second model is the best in terms of rmse
# this is because it has the best bias/var tradeoff
