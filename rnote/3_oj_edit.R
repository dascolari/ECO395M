...
## read in the data
oj = read.csv("/Users/davidscolari/Dropbox/grad_school/3_Spring22/datmin/ECO395M/data/oj.csv") 
head(oj)
...
# Option 1: main effect only (shift intercept)
reg_ads = lm(logmove ~ log(price)*brand + feat, data=oj)
summary(reg_ads)

# Option 2: two-way interaction (allow change in slope (change in partial effect) across values of brand/feature)
reg_ads2 = lm(logmove ~ log(price)*(brand+feat), data=oj)
summary(reg_ads2)

# Option 3: three-way interaction (all changes in slope and well as )
