...
# you shouldn't do this, you gotta scale by training data
# use that scaling, not scaling by the entire dataset
fgl_scale = fgl %>%
...
# also knn3 counts, not averages the k nearest neighbors
...
ggplot(data=fgl_test) +
	geom_point(aes(x=RI, y=Mg, shape=type, color = type), size=2)
...
#he divided some stuf at the end, not sure what it was
#some kind of performance rate
#multinomial logit is in the classification slides